...
local ToolListCodeDefinitions = require(script.Parent.tools.list_code_definition_names)
...
local FAILED_ASSET_IDS = _G.__VECTOR_FAILED_ASSET_IDS
if type(FAILED_ASSET_IDS) ~= "table" then
    FAILED_ASSET_IDS = {}
    _G.__VECTOR_FAILED_ASSET_IDS = FAILED_ASSET_IDS
end

local GAME_CREATOR_ID = tonumber(game.CreatorId) or 0
local GAME_CREATOR_TYPE = tostring(game.CreatorType)

-- TEMP: Restrict to Roblox-owned assets only
local ROBLOX_ONLY = false
...
local function captureSceneSnapshot()
    local maxNodes = 400
    local queue = {}
    local results = {}
    local seen = {}
    ...
    while #queue > 0 and #results < maxNodes do
        local inst = table.remove(queue, 1)
        if inst ~= game then
            local okPath, fullName = pcall(function() return inst:GetFullName() end)
            if okPath and type(fullName) == "string" then
                local node = {
                    path = fullName,
                    className = inst.ClassName,
                    name = inst.Name,
                ...
                if parent then
                    local okParent, parentPath = pcall(function() return parent:GetFullName() end)
                    if okParent and type(parentPath) == "string" then
                ...
                if inst:IsA("BasePart") then
                    ...
                    props.Size = serializeVector3(inst.Size)
                    props.Position = serializeVector3(inst.Position)
                end
                if inst:IsA("Model") and inst.PrimaryPart then
                    local okPrimary, primaryPath = pcall(function() return inst.PrimaryPart:GetFullName() end)
                    if okPrimary and type(primaryPath) == "string" then
                        props.PrimaryPart = { __t = "Instance", path = primaryPath }
                    end
                end
                node.props = props
        ...
        if #results >= maxNodes then break end

        local children = inst:GetChildren()
        for _, child in ipairs(children) do
            enqueue(child)
            if #results + #queue >= maxNodes then break end
        end
    end
...
local function captureCodeDefinitionSnapshot()
	local okPermission = ensurePermissionWithStatus()
	if not okPermission then
		return nil
	end
	local ok, defs = pcall(function()
		return ToolListCodeDefinitions({ limit = 400, maxScripts = 400, maxLinesPerScript = 4000 })
	end)
...
local function fetchAssets(query, limit, tags)
	local base = getBackendBaseUrl()
	...
	if typeof(tags) == "table" then
		for _, tag in ipairs(tags) do
			if type(tag) == "string" and #tag > 0 then
				url = string.format("%s&tag=%s", url, HttpService:UrlEncode(tag))
			end
		end
	end
	...
	local raw = json.results or {}
	local filtered = {}
	for _, entry in ipairs(raw) do
		local assetId = assetIdKey(entry and (entry.id or entry.assetId or entry.AssetId))
		if assetId and assetId > 0 and assetId < 100000000000 then
			local typeName = nil
			if typeof(entry.type) == "string" then
				typeName = string.lower(entry.type)
			elseif typeof(entry.AssetType) == "string" then
				typeName = string.lower(entry.AssetType)
			elseif typeof(entry.assetType) == "string" then
				typeName = string.lower(entry.assetType)
			elseif typeof(entry.assetType) == "number" then
				typeName = tostring(entry.assetType)
			end
			local isModelLike = typeName == nil or string.find(typeName, "model", 1, true) ~= nil or typeName == "8"
			if isModelLike then
				entry.id = assetId
...
local function assetIdKey(assetId)
    if typeof(assetId) == "number" then
        return assetId
    end
    local asNumber = tonumber(assetId)
    if typeof(asNumber) == "number" and asNumber == asNumber then
        return asNumber
    end
    return nil
end

local function recordAssetFailure(assetId, message)
    local key = assetIdKey(assetId)
    if not key then return end
    FAILED_ASSET_IDS[key] = message or true
end

local function clearAssetFailure(assetId)
    local key = assetIdKey(assetId)
    if not key then return end
    FAILED_ASSET_IDS[key] = nil
end

local function isRobloxCreatorName(name)
    if typeof(name) ~= "string" then return false end
    local lower = string.lower(name)
    return lower == "roblox" or lower == "roblox team" or lower == "roblox official" or string.sub(lower, 1, 7) == "roblox "
end

local function isRobloxEntry(entry)
    local creatorName = (typeof(entry.creator) == "string" and entry.creator)
        or (typeof(entry.CreatorName) == "string" and entry.CreatorName) or ""
    ...
    elseif typeof(entry.CreatorType) == "EnumItem" then
        creatorType = string.lower(tostring(entry.CreatorType))
    end
    if creatorType then
        local trimmed = string.match(creatorType, "([^.]+)$")
        if trimmed then creatorType = trimmed end
    end
    if isRobloxCreatorName(creatorName) then return true end
    if creatorId == 1 then return true end
    if creatorType == "roblox" then return true end
    return false
end
...
local function computeCatalogScore(entry, index)
    ...
    local isVerified = entry.isVerified == true or entry.IsVerified == true or entry.creatorHasVerifiedBadge == true

    local assetId = assetIdKey(entry.id or entry.AssetId)
    if assetId and FAILED_ASSET_IDS[assetId] then
        score += 100000
    end

    if isRobloxCreatorName(creatorName) or (creatorType == "roblox") or (creatorId == 1) then
        score = score - 8000
    end

    if creatorId and creatorId == GAME_CREATOR_ID then
        score = score - 6000
    elseif GAME_CREATOR_TYPE == tostring(Enum.CreatorType.Group) and creatorType == "group" and creatorId == GAME_CREATOR_ID then
        score = score - 4000
    end
...
local function sortCatalogResults(results)
    local scored = {}
    -- Optional strict Roblox-only filter
    if ROBLOX_ONLY then
        local filtered = {}
        for _, e in ipairs(results) do
            if isRobloxEntry(e) then table.insert(filtered, e) end
        end
        results = filtered
    end
    for index, entry in ipairs(results) do
        table.insert(scored, { entry = entry, score = computeCatalogScore(entry, index) })
    end
    table.sort(scored, function(a, b)
        if a.score == b.score then return (a.entry.id or a.entry.AssetId or 0) < (b.entry.id or b.entry.AssetId or 0) end
        return a.score < b.score
    end)
    local ordered = {}
    for _, info in ipairs(scored) do
        table.insert(ordered, info.entry)
    end
    return ordered
end
...
local function insertAsset(assetId, parentPath)
    ...
    if FAILED_ASSET_IDS[key] then
        return false, FAILED_ASSET_IDS[key]
    end
	...
	if not ok then
        recordAssetFailure(assetId, tostring(modelOrErr))
		return false, modelOrErr
	end
    clearAssetFailure(assetId)
	return true, modelOrErr
end

local function insertAssetFromFreeModels(query, parentPath, relativePosition, opts)
	if typeof(query) ~= "string" or #query == 0 then
	...
	local origin = cam and cam.CFrame and cam.CFrame.Position or Vector3.new()
	local offset
	if typeof(relativePosition) == "Vector3" then
		offset = relativePosition
	elseif cam and cam.CFrame then
		offset = cam.CFrame.LookVector * 16
	else
		offset = Vector3.new()
	end

    local maxPages = 5 -- broaden search depth to improve odds
    if opts and typeof(opts.maxPages) == "number" then
        maxPages = math.clamp(math.floor(opts.maxPages), 1, 10)
    end

    local candidates = {}
    local seenAsset = {}
    for pageIndex = 0, maxPages - 1 do
        local okPage, pages = pcall(function()
            return InsertService:GetFreeModels(query, pageIndex)
        end)
        if okPage then
            local page = (pages or {})[1]
            if page and typeof(page) == "table" and typeof(page.Results) == "table" then
                for resultIndex, entry in ipairs(page.Results) do
            ...
            warn(string.format("[Vector] GetFreeModels failed page=%d err=%s", pageIndex, tostring(pages)))
        end
    end
    ...
    if #candidates == 0 then
        local msg = string.format("Failed to find \"%s\" in the marketplace!", query)
        warn(msg)
        return false, msg
    end

    table.sort(candidates, function(a, b)
        local scoreA = a.__score or 0
        local scoreB = b.__score or 0
        if scoreA == scoreB then
            return (assetIdKey(a.AssetId) or 0) < (assetIdKey(b.AssetId) or 0)
        end
	...
	for _, entry in ipairs(candidates) do
		local assetId = entry and assetIdKey(entry.AssetId)
		if assetId and not FAILED_ASSET_IDS[assetId] then
			...
			if okLoad and container then
				...
				pcall(function()
					if typeof(entry.Name) == "string" and #entry.Name > 0 then
				...
				if container ~= model then pcall(function() container:Destroy() end) end
                clearAssetFailure(assetId)
				return true, model, assetId
			else
				local errText = okLoad and "load_failed" or container
				lastErr = errText
                recordAssetFailure(assetId, tostring(errText))
			end
		else
			lastErr = lastErr or "cached_failure"
		end
	end
	local errMsg = lastErr and tostring(lastErr) or string.format("No insertable free model found for \"%s\"", query)
	warn(errMsg)
	return false, errMsg
end
...
local function buildUI(gui)
    ...
    -- append a styled text block (assistant/plain text) in the transcript area
    local function uiAddTextBlock(text)
        statusFrame.Visible = true
        local card = Instance.new("Frame")
    ...
    -- compact tool summary block (used in Ask mode)
    local function uiAddToolSummary(p)
        local title = summarizeProposal(p)
        local details = p.notes or ""
        statusFrame.Visible = true
        ...
        local pad = Instance.new("UIPadding"); pad.PaddingLeft = UDim.new(0, 8); pad.PaddingRight = UDim.new(0, 8); pad.PaddingTop = UDim.new(0, 6); pad.PaddingBottom = UDim.new(0, 6); pad.Parent = row
    ...
    local ui = {
        ...
        addText = uiAddTextBlock,
        addToolChip = uiAddToolSummary,
	...
	if type(results) == "table" then
		ordered = sortCatalogResults(results)
	else
		ordered = results
	end
	local layout = Instance.new("UIListLayout")
	...
	for i, p in ipairs(proposals) do
		...
		if p.type == "asset_op" and p.search then
			...
			browseBtn.MouseButton1Click:Connect(function()
				...
				local ok, resultsOrErr = fetchAssets(p.search.query or "", p.search.limit or 8, p.search.tags)
				if ok then
...
local gui = nil
    ...
    -- Render compact assistant text if a final message line is streamed
    if string.find(lowerLine, "assistant.final ") then
        local msg = string.gsub(line, "^assistant.final%s+", "")
        if _G.__VECTOR_UI and _G.__VECTOR_UI.addText then
            _G.__VECTOR_UI.addText(msg)
        end
    end
    if string.find(lowerLine, "assistant.start ") then
        local msg = string.gsub(line, "^assistant.start%s+", "")
        if _G.__VECTOR_UI and _G.__VECTOR_UI.addText then _G.__VECTOR_UI.addText(msg) end
    end
    if string.find(lowerLine, "assistant.update ") then
	...
	elseif p.type == "asset_op" then
		if p.insert and p.insert.assetId then
            ...
            if _G.__VECTOR_AUTO then
                    ...
                    if ROBLOX_ONLY then ui.addStatus("asset.filter roblox_only") end
                    -- Fetch a broader candidate set from backend; we will filter/order locally
                    local desiredLimit = tonumber(p.search.limit) or 0
                    local fetchLimit = math.max(60, desiredLimit, 0)
                    local okFetch, resultsOrErr = fetchAssets(query, fetchLimit, p.search.tags)
                    if okFetch and type(resultsOrErr) == "table" then
						...
						for _, entry in ipairs(orderedResults) do
							local assetId = tonumber(entry and entry.id)
							if assetId then
								if FAILED_ASSET_IDS[assetId] then
									ui.addStatus("auto.asset_search skip cached " .. tostring(assetId))
									lastErr = FAILED_ASSET_IDS[assetId]
									continue
								end
						...
						if not inserted and typeof(query) == "string" and #query > 0 then
							ui.addStatus("auto.asset_search fallback free model")
							local okFallback, modelOrErr, fallbackAssetId = insertAssetFromFreeModels(query, desiredParentPath, nil, { maxPages = 3 })
							if okFallback then
								_G.__VECTOR_LAST_ASSET_ERROR = nil
								local insertedPath = nil
								if modelOrErr and typeof(modelOrErr) == "Instance" and modelOrErr.GetFullName then
								...
								ui.addStatus("auto.ok asset (fallback)")
								reportApply(p.id, { ok = true, type = p.type, op = "search_insert_fallback", assetId = fallbackAssetId, insertedPath = insertedPath, query = query, parentPath = desiredParentPath })
								return true
							else
								local errMsg = tostring(modelOrErr)
								lastErr = lastErr or errMsg
								_G.__VECTOR_LAST_ASSET_ERROR = errMsg or "insert_failed"
								ui.addStatus("auto.err asset " .. errMsg)
								reportApply(p.id, { ok = false, type = p.type, op = "search_insert_fallback", query = query, parentPath = desiredParentPath, error = errMsg })
							end
						end
...
local function maybeAutoContinue(workflowId)
	...
	local function runSend()
		...
		local ctx = buildContextSnapshot()
	...
	ui.nextBtn.MouseButton1Click:Connect(function()
		...
		local mode = CURRENT_MODE
		local opts = { mode = mode, maxTurns = (mode == "ask") and 1 or nil, enableFallbacks = true, modelOverride = getModelOverride(), autoApply = _G.__VECTOR_AUTO }
		local resp = sendChat("local", followup, buildContextSnapshot(), lastWorkflowId, opts)
		if not resp.Success then return end
		local ok, parsed = pcall(function() return HttpService:JSONDecode(resp.Body) end)
		if not ok or parsed.error then return end
		renderProposals(ui.list, parsed.proposals or {})
