...
local function ensureParentExists(path)
    if type(path) ~= "string" or #path == 0 then return nil end

    local function tokenize(full)
        local tokens = {}
        local buf, inBr = {}, false
    ...
    for i = 2, #tokens do
        ...
        if not child then
            if current == workspace or current:IsDescendantOf(workspace) then
...
return function(className, parentPath, props)
    ...
    local parent = resolveByFullName(parentPath) or ensureParentExists(parentPath) or resolveByFullName(parentPath)
    if not parent then
