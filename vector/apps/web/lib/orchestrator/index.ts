...
const SYSTEM_PROMPT = `You are Vector, a Roblox Studio copilot.
    ...
    - Proposal-first and undoable: never change code/Instances directly; always propose a small, safe step the plugin
    can preview/apply.
    - No prose, no markdown, no code fences, no extra tags. Do NOT invent fictitious tags like <plan> or <thoughts>.
        ...
        - Objects/arrays: INNER TEXT MUST be strict JSON (double quotes; no trailing commas). Never wrap JSON in quotes.
        ...
        - Actions: show_diff(path, edits[]), apply_edit(path, edits[]), create_instance(className, parentPath, props ?),
        set_properties(path, props), rename_instance(path, newName), delete_instance(path), search_assets(query, tags ?,
        limit ?), insert_asset(assetId, parentPath ?), generate_asset_3d(prompt, tags ?, style ?, budget ?).
        ...
        - Wrappers with "__t": Vector3 { "__t": "Vector3", "x": 0, "y": 1, "z": 0 }; Vector2 { "__t": "Vector2", "x": 0,
        "y": 0 };
        ...
        - Use that selection as <parentPath> for create_instance / insert_asset when unspecified.
          - Use that selection's path for rename_instance/set_properties/delete_instance when <path> is missing.
            ...
            - Only set real class properties in <props>.If a key is not a property of the target class, write it as an
              Attribute by prefixing with "@".
              - Do NOT rename via set_properties.Use < rename_instance> instead.
                ...
                - Never delete DataModel or Services.Operate under Workspace unless the user targets another service
                explicitly.
                - Use only valid Roblox class names for <className>.If unsure, prefer "Part" or "Model".
                  ...
                  - When a segment contains dots / brackets, use bracket notation exactly:
                  game.Workspace["My.Part"]["Wall [A]"]

                  Examples(correct)
                  <create_instance>\n < className> Part < /className>\n <parentPath>game.Workspace</parentPath>\n <
                          props> { "Name": "Cell_1_1", "Anchored": true, "Size": { "__t": "Vector3", "x": 4, "y": 1,
                          "z": 4 }, "CFrame": { "__t": "CFrame", "comps": [0, 0.5, 0, 1, 0, 0, 0, 1, 0, 0, 0, 1] } } <
                            /props>\n</create_instance>
...
function coercePrimitive(v: string): any {
  ...
  // JSON5-like fallback for common LLM outputs: single quotes, unquoted keys, trailing commas, fenced
  // code
  if (
...
function parseToolXML(
  text: string,
): { name: string; args: Record<string, any> } | null {
  if (!text) return null;

  const toolMatch = text.match(/<([a-za-z_][\w]*)>([\s\s]*)<\/\1>/); //prettier - ignore
  if (!toolMatch) return null;
  const name = toolMatch[1];
  const inner = toolMatch[2];
  // parse child tags into args
  const args: Record<string, any> = {};
  const tagRe = /<([a-zA-Z_][\w]*)>([\s\S]*?)<\/\1>/g; //prettier - ignore
  ...
  if (Object.keys(args).length === 0) {
    ...
    if (asJson && typeof asJson === "object")
      return { name, args: asJson as any };
  }
  return { name, args };
}
...
function toEditArray(editsRaw: any): Edit[] | null {
  ...
  for (const e of parsed) {
    if (
      ...
      typeof e.start.line === "number" &&
      typeof e.start.character === "number" &&
      typeof e.end.line === "number" &&
      typeof e.end.character === "number"
    ) {
      out.push({
  ...
  for (let i = 1; i < out.length; i++) {
    ...
    const overlaps =
      ...
      (cur.start.line === prev.end.line &&
        cur.start.character < prev.end.character);

    if (overlaps) return null;
  }

  // budget caps: max 20 edits, 2000 inserted chars
  const totalInsertChars = out.reduce(
    (n, e) => n + (e.text ? String(e.text).length : 0),
    0,
  );
...
function mapToolToProposals(
  ...
  if (name === "create_instance") {
    ...
    if (typeof (a as any).className === "string" && parentPath) {
      const op: ObjectOp = {
        op: "create_instance",
        className: (a as any).className,
        parentPath,
        props: (a as any).props,
      };
      proposals.push({
        id: id("obj"),
        type: "object_op",
        ops: [op],
        notes: "Parsed from create_instance",
      });
      return proposals;
    }
  }
  ...
  if (name === "set_properties") {
    if (
      ...
      proposals.push({
        ...
        type: "object_op",
        ops: [op],
        notes: "Parsed from set_properties",
      });
      return proposals;
    }
  }
        ...
        ops: [
          {
            op: "rename_instance",
            path,
            newName: (a as any).newName,
          },
        ],
        notes: "Parsed from rename_instance",
      });
      return proposals;
    }
  }
  ...
  if (name === "delete_instance") {
    ...
    const path = null;
    console.log("selPath not found");
    if (path) {
      // Guard: avoid destructive deletes at DataModel or Services level
      if (/^game(\.[A-Za-z]+Service|\.DataModel)?$/.test(path))
        return proposals;
      proposals.push({
        id: id("obj"),
        type: "object_op",
        ...
        notes: "Parsed from delete_instance",
      });
      return proposals;
    }
  }
  ...
  if (name === "search_assets") {
    const query =
      typeof (a as any).query === "string" ? (a as any).query : msg || "button";
    const tags = Array.isArray((a as any).tags)
      ? (a as any).tags.map(String)
      : undefined;
  ...
  if (name === "insert_asset") {
    const assetId =
      typeof (a as any).assetId === "number"
        ? (a as any).assetId
        : Number((a as any).assetId);
    if (!isNaN(assetId)) {
      proposals.push({
        id: id("asset"),
        type: "asset_op",
        insert: {
          assetId,
          parentPath:
            typeof (a as any).parentPath === "string"
              ? (a as any).parentPath
              : undefined,
        },
      });
    ...
    if (typeof (a as any).prompt === "string") {
      proposals.push({
        ...
        generate3d: {
          ...
          tags: Array.isArray((a as any).tags)
            ? (a as any).tags.map(String)
            : undefined,
          style:
            typeof (a as any).style === "string" ? (a as any).style : undefined,
          budget:
            typeof (a as any).budget === "number"
              ? (a as any).budget
              : undefined,
        },
      });
...
export async function runLLM(input: ChatInput): Promise<Proposal[]> {
  ...
  const selPath =
    input.context.selection && input.context.selection.length === 1
      ? input.context.selection[0].path
      : undefined;

  const selIsContainer =
    !!selPath &&
    /^(?:game\.(?:Workspace|ReplicatedStorage|ServerStorage|StarterGui|StarterPack|StarterPlayer|Lighting|Teams|SoundService|TextService|CollectionService)|game\.[A-Za-z]+\.[\s\S]+)/.test(
      selPath,
    );
  ...
  const providerRequested = !!(
    input.provider &&
    input.provider.name === "openrouter" &&
    !!input.provider.apiKey
  );
  const useProvider =
    providerRequested || process.env.VECTOR_USE_OPENROUTER === "1";
  const streamKey = (input as any).workflowId || input.projectId;

  pushChunk(
    streamKey,
    `orchestrator.start provider=${useProvider ? "openrouter" : "fallback"} mode=${
      input.mode || "agent"
    }`,
  );

  console.log(
    `[orch] start provider=${useProvider ? "openrouter" : "fallback"} mode=${
      input.mode || "agent"
    } msgLen=${msg.length}`,
  );
  ...
  const addObj = (ops: ObjectOp[], notes?: string) =>
    proposals.push({ id: id("obj"), type: "object_op", ops, notes });
          ...
          [
            {
              ...
              className: "Part",
              parentPath: basePath,
              props: makePartProps(name, x, 0.5, z),
            },
          ],
          `Create ${name}`,
        );
  ...
  if (/\bfarming\b/.test(lower)) {
    ...
    // 4x4 soil grid = 16 steps including farm + base → add 14 more
  ...
  // === Helper: build tool XML for assistant history ===
  ...
  if (useProvider) {
    ...
    for (let turn = 0; turn < maxTurns; turn++) {
      ...
      try {
        ...
        pushChunk(
          streamKey,
          `provider.response turn=${turn} chars=${content.length}`,
        );
        console.log(
          `[orch] provider.ok turn=${turn} contentLen=${content.length}`,
        );
      } catch (e: any) {
      ...
      if (
        (name === "show_diff" || name === "apply_edit") &&
        !a.path &&
        input.context.activeScript?.path
      ) {
      ...
      if (schema) {
        ...
        if (!parsed.success) {
          ...
          console.warn(
            `[orch] validation.error tool=${String(name)} ${errMsg}`,
          );

          // Reflect error and retry
          messages.push({ role: "assistant", content: toXml(String(name), a) });
          messages.push({
            role: "user",
            content: `VALIDATION_ERROR ${String(name)}\n${errMsg}`,
          });

          if (consecutiveValidationErrors > validationRetryLimit) {
            throw new Error(
              `Validation failed repeatedly for ${String(name)}: ${errMsg}`,
            );
          }
          continue;
        } else {
      ...
      if (isContextTool) {
        ...
        const safeResult =
          ...
          typeof (result as any).text === "string"
            ? {
                ...(result as any),
                text: ((result as any).text as string).slice(0, 40000),
              }
            : result;
        ...
        messages.push({
          role: "user",
          content: `TOOL_RESULT ${String(name)}\n${JSON.stringify(safeResult)}`,
        });
      ...
      if (mapped.length) {
        pushChunk(
          streamKey,
          `proposals.mapped ${String(name)} count=${mapped.length}`,
        );
        console.log(
          `[orch] proposals.mapped tool=${String(name)} count=${mapped.length}`,
        );
        return mapped;
      }

      // Unknown tool: reflect error and retry
  ...
  const fallbacksEnabled =
    typeof (input as any).enableFallbacks === "boolean"
    ...
    return [
      {
        ...
        preview: { unified },
        safety: {
          beforeHash: (require("node:crypto") as typeof import("node:crypto"))
            .createHash("sha1")
            .update(old)
            .digest("hex"),
        },
    ...
    return [
      {
        id: id("obj"),
        type: "object_op",
        notes: "Rename selected instance by appending Vector",
        ops: [
          {
            op: "rename_instance",
            path: first.path,
            newName: `${first.path.split(".").pop() || "Instance"}_Vector`,
          },
        ],
      },
    ];
